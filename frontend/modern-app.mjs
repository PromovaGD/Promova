...
import { modernApi as api } from "./services/modern-api.mjs";
import { canAccess, canonicalUrl, legacyDestination, matchRoute, roleHome, safeReturnTo } from "./router.mjs";
import { escapeHtml as esc } from "./utils/html.mjs";

const state = { user: loadAuthUser(), validated: false, route: null, data: null, error: null, loading: false, epoch: 0, controller: null, drafts: new Map(), shellRole: null };
...
export async function startModernApp(root) {
  document.body.classList.add("modern-ui");
  root.innerHTML = `<a class="skip-link" href="#main-content">Pular para o conteúdo</a><div id="modern-root"></div><div id="toast" class="toast" role="status" aria-live="polite"></div>${dialogMarkup()}`;
  ...
  const legacy = legacyDestination(location);
...
function render() {
  const mount = document.querySelector("#modern-root");
  if (!mount) return;
...
function careerPlan(manager) {
  ...
  return `${manager?personHeader(person):""}${tabs([[`${base}?view=objectives`,"Objetivos"],[`${base}?view=context`,"Contexto"]],view)}${view==="context"?careerContext(profile,manager):objectives(profile,manager,person)}`;
}
...
function careerContext(profile,manager) { const role=profile.jobRole; if(!manager) return panel("Contexto de carreira",`<dl class="details"><div><dt>Cargo</dt><dd>${esc(role?.name||"Não definido")}</dd></div><div><dt>Progressão</dt><dd>${esc(profile.currentLevel||"—")} → ${esc(profile.targetLevel||"—")}</dd></div><div><dt>Características</dt><dd>${(profile.characteristics||[]).map(x=>`<span class="tag">${esc(x)}</span>`).join(" ")||"Não definidas"}</dd></div></dl><p class="note">Contexto atual do plano. Resultados históricos preservam o contexto do momento da análise.</p>`,"","fill"); return panel("Editar contexto",`<form id="plan-form" class="form-grid"><label>Cargo<select name="jobRoleId">${(state.data[2].activeRoles||[]).map(r=>`<option value="${r.id}" ${sel(role?.id,r.id)}>${esc(r.name)}</option>`).join("")}</select></label><label>Nível atual<select name="currentLevel">${levelOptions(profile,profile.currentLevel)}</select></label><label>Nível alvo<select name="targetLevel">${levelOptions(profile,profile.targetLevel)}</select></label><label class="wide">Características<textarea name="characteristics" rows="6" maxlength="2000">${esc((profile.characteristics||[]).join("\n"))}</textarea></label><input type="hidden" name="expectedUpdatedAt" value="${esc(profile.updatedAt||"")}"></form>`,`<button class="button primary" type="submit" form="plan-form">Salvar contexto</button>`,"fill"); }

function objectiveEditor(person,profile){ const current=state.route.id==="objective"?(profile.objectives||[]).find(o=>o.id===state.route.params.objectiveId):null; return `${breadcrumbs([[`/manage/people/${person.id}/career-plan`,person.name]])}${panel(current?"Editar objetivo":"Novo objetivo",`<form id="objective-form" class="form-grid"><label class="wide">Objetivo<textarea name="text" rows="6" maxlength="2000" required>${esc(current?.text||draft("objective")?.text||"")}</textarea><small>Máximo de 2.000 caracteres</small></label><label>Status<select name="status">${["ACTIVE","COMPLETED","ARCHIVED"].map(s=>`<option ${sel(current?.status||"ACTIVE",s)} value="${s}">${labelStatus(s)}</option>`).join("")}</select></label><label>Data alvo<input type="date" name="targetDate" value="${esc(current?.targetDate||"")}"></label><input type="hidden" name="expectedUpdatedAt" value="${esc(current?.updatedAt||"")}"></form>`,`<a class="button" data-link href="/manage/people/${person.id}/career-plan">Cancelar</a><button class="button primary" type="submit" form="objective-form">Salvar objetivo</button>`,"fill")}`; }
...
function githubImport(){return panel("Localizar pull requests",`<form id="github-import-form" class="form-grid"><label>Repositório<input name="repoSlug" placeholder="organizacao/repositorio" required></label><label>Autor<input name="authorLogin" placeholder="login-github" required></label></form><div id="github-results">${empty("Informe um repositório e autor para pesquisar.")}</div>`,`<button class="button primary" type="submit" form="github-import-form">Pesquisar</button>`,"fill");}
...
async function onClick(event){const link=event.target.closest("a[data-link]");if(link&&!event.defaultPrevented&&!event.metaKey&&!event.ctrlKey&&!event.shiftKey&&event.button===0){event.preventDefault();if(await allowNavigation())go(link.href);return;}const button=event.target.closest("button[data-action],button[data-review]");if(!button)return;const action=button.dataset.action;try{if(action==="open-menu")return openMenu(button);if(action==="close-menu")return closeMenu();if(action==="logout"){await logoutUser();clearAuthSession();state.user=null;state.drafts.clear();return go("/", "replace");}if(action==="retry")return navigate(location.href,{history:"none"});if(action==="account")return showDialog("Sua conta",`<p><strong>${esc(state.user.name)}</strong><br>${esc(state.user.email)}<br>${state.user.role==="MANAGER"?"Gestor":"Funcionário"}</p>`,[{label:"Fechar"}]);if(action==="clear-analyses")return confirmAction("Limpar histórico", "As análises salvas e suas revisões serão removidas. As evidências capturadas serão preservadas.", async()=>{await api.clearAnalyses(state.route.query);toast("Histórico removido.");navigate(location.href,{history:"none"});});if(action==="dismiss-evidence")return confirmAction("Dispensar evidência", "A fonte será preservada e sairá das pendências. O fluxo atual não oferece restauração.", async()=>{await api.dismissEvidence(state.data.id);state.drafts.delete(`evidence:${state.data.id}`);go("/app/inbox?status=pending","replace");});if(action==="analyze-evidence"){disable(button,"Analisando…");const result=await api.analyzeEvidence(state.data.id,document.querySelector("#observation")?.value);state.drafts.delete(`evidence:${state.data.id}`);return go(`/workspace/people/${state.user.id}/analyses/${result.analysisId}`,"replace");}if(action==="sync-github"){disable(button,"Sincronizando…");const result=await api.syncGithub();toast(`${result.created} novas; ${result.existing} já existentes.`);return navigate(location.href,{history:"none"});}if(action==="test-github"){disable(button,"Testando…");await api.testGithub();toast("Conexão validada.");button.disabled=false;button.textContent="Testar conexão";}if(action==="disconnect-github")return confirmAction("Desconectar GitHub", "A configuração será removida. Evidências já capturadas permanecem.",async()=>{await api.clearGithub();navigate(location.href,{history:"none"});});if(action==="archive-role")return confirmAction("Arquivar cargo", "Se houver pessoas atribuídas, o servidor exigirá um cargo substituto compatível.",async()=>{await api.archiveRole(state.route.params.roleId,null);go("/manage/career/roles","replace");});if(button.dataset.review){disable(button,"Registrando…");const form=new FormData(document.querySelector("#review-form"));await api.review(state.route.params.ownerId,state.route.params.analysisId,{status:button.dataset.review,comment:form.get("comment")||null,expectedLatestReviewId:numberOrNull(form.get("expectedLatestReviewId")),idempotencyKey:crypto.randomUUID()});state.drafts.delete(`review:${state.route.params.analysisId}`);toast("Revisão registrada.");return go(`${location.pathname}?view=history`,"replace");}}catch(error){button.disabled=false;toast(error.message||"A operação não foi concluída.",true);}}

async function onSubmit(event){event.preventDefault();const form=event.target,fd=new FormData(form);try{setFormBusy(form,true);if(form.dataset.authForm){const response=form.dataset.authForm==="login"?await loginUser({email:fd.get("email"),password:fd.get("password")}):await registerUser({name:fd.get("name"),email:fd.get("email"),password:fd.get("password")});saveAuthSession(response.token,response.user);state.user=response.user;const requested=safeReturnTo(new URLSearchParams(location.search).get("returnTo"));return go(requested&&canAccess(matchRoute(requested),state.user)?requested:roleHome(state.user),"replace");}if(form.id==="inbox-filter"||form.id==="analyses-filter"||form.id==="reports-filter"||form.id==="framework-filter"||form.id==="people-filter"||form.id==="reviews-filter")return applyFilters(form);if(form.id==="plan-form"){await api.updatePlan(state.route.params.employeeId,{jobRoleId:Number(fd.get("jobRoleId")),currentLevel:fd.get("currentLevel"),targetLevel:fd.get("targetLevel"),characteristics:String(fd.get("characteristics")||"").split("\n").map(x=>x.trim()).filter(Boolean),expectedUpdatedAt:fd.get("expectedUpdatedAt")||null});toast("Contexto salvo.");return navigate(location.href,{history:"none"});}if(form.id==="objective-form"){const body={text:fd.get("text"),status:fd.get("status"),targetDate:fd.get("targetDate")||null,expectedUpdatedAt:fd.get("expectedUpdatedAt")||null};if(state.route.id==="objective-new")await api.createObjective(state.route.params.employeeId,body);else await api.updateObjective(state.route.params.employeeId,state.route.params.objectiveId,body);state.drafts.delete("objective");toast("Objetivo salvo.");return go(`/manage/people/${state.route.params.employeeId}/career-plan`,'replace');}if(form.id==="terminology-form"){await api.terminology(Object.fromEntries(fd));toast("Terminologia salva.");return navigate(location.href,{history:"none"});}if(form.id==="role-form"){const body={name:fd.get("name"),description:fd.get("description"),allowedLevelIds:fd.getAll("allowedLevelIds")};if(state.route.id==="role-new")await api.createRole(body);else await api.updateRole(state.route.params.roleId,body);toast("Cargo salvo.");return go("/manage/career/roles","replace");}if(form.id==="github-form"){await api.saveGithub({repoSlug:fd.get("repoSlug"),authorLogin:fd.get("authorLogin")});toast("Configuração salva.");return navigate(location.href,{history:"none"});}if(form.id==="github-import-form")return searchGithub(form,fd);}catch(error){toast(error.message||"Não foi possível salvar. Seus dados permanecem no formulário.",true);}finally{setFormBusy(form,false);}}

function onInput(event){const el=event.target;if(el.id==="observation"){state.drafts.set(`evidence:${state.data.id}`,{observation:el.value});count(el);}if(el.form?.id==="review-form")state.drafts.set(`review:${state.route.params.analysisId}`,{comment:el.form.elements.comment.value});if(el.form?.id==="objective-form")state.drafts.set("objective",Object.fromEntries(new FormData(el.form)));}
function onChange(event){if(event.target.closest(".filters")?.tagName==="FORM")event.target.form.requestSubmit();}
...
function redirectToLogin(){const value=encodeURIComponent(`${location.pathname}${location.search}`);history.replaceState({},"",`/login?returnTo=${value}`);state.route=matchRoute(location);state.loading=false;render();}
function expireSession(){state.user=null;state.validated=false;state.data=null;state.drafts.clear();redirectToLogin();}
function renderForbidden(){document.querySelector("#modern-root").innerHTML=shell(`<section class="workspace">${publicState("Permissão necessária",state.error)}</section>`);}
...
function setFormBusy(form,busy){form.querySelectorAll("button,input,select,textarea").forEach(el=>{if(el.type!=="hidden")el.disabled=busy});}
...
async function searchGithub(form,fd){const parts=String(fd.get("repoSlug")).split("/").filter(Boolean);if(parts.length!==2)throw new Error("Use o formato organização/repositório.");const result=await api.searchGithub(parts[0],parts[1],`author:${fd.get("authorLogin")}`,1);const root=document.querySelector("#github-results"),items=result.items||[];root.innerHTML=items.length?`<div class="row-list">${items.map(pr=>`<button class="row" type="button" data-import-pr="${pr.number}" data-repo="${esc(fd.get("repoSlug"))}"><div><strong>PR #${pr.number} · ${esc(pr.title)}</strong><small>${esc(pr.user?.login||pr.authorLogin||"")}</small></div><span>Importar</span></button>`).join("")}</div>`:empty("Nenhum pull request encontrado.");root.querySelectorAll("[data-import-pr]").forEach(b=>b.addEventListener("click",async()=>{disable(b,"Importando…");try{const e=await api.captureGithub({repo:b.dataset.repo,pullNumber:Number(b.dataset.importPr),usernameHint:fd.get("authorLogin")});go(`/workspace/people/${state.user.id}/evidence/${e.id}`);}catch(error){b.disabled=false;toast(error.message,true);}}));}
